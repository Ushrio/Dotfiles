...
while IFS= read -r i; do
    if [[ -f "$i" ]]; then
        ...
        incomplete_icon_path=$(find "$i" "$directory" | head -1)
...
selection=$(echo -ne "$final_rofi" | rofi -markup-rows -theme ~/.config/rofi/themes/notif.rasi -dmenu -eh 3 -a "$active_list" -u "$urgent_list" -sep '\x0f' -p "Notification Center" -no-fixed-num-lines -lines 12 -i -no-config)
